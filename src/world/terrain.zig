...
pub const Terrain = struct {
    ...
    pub fn init() !Self {
        ...
        for (0..self.chunks.len) |key| {
            _ = try workerFunction(&self.chunks, key, viewDistance);
    ...
    pub fn optimize(self: *Self, blocks: *[65536]custom_block, chunkx: usize, chunkz: usize) Retour {
        ...
        var arraybuffer: []u32 = allocator.alloc(u32, len * 2 * 4 * 128) catch |err| {
            std.debug.print("Trying allocate chunk : {any}\n", .{err});
            return undefined;
        };

        var indicebuffer: []u32 = allocator.alloc(u32, len * 6 * 128) catch |err| {
            std.debug.print("Trying allocate chunk : {any}\n", .{err});
            return undefined;
        };
        ...
        for (self.chunks) |*chunk| {
            if (chunk.loaded) {
                if (chunk.uploaded) {
                    ...
                    chunk.draw(self.program, ntexture);
                } else {
                    // std.debug.print("{s} {any} {any}\n", .{ "up", chunk.x, chunk.y });
